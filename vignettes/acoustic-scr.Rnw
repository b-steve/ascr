...
\item First detected calls \scp[e.g.,][]{Dawson2009, Stevenson2016}.
\end{itemize}
...
Unfortunately the above data cannot be made available. The halfnormal
detection function will be selected by default, but this can be
changed using the \texttt{detfn} argument.
...
However, \sct{Stevenson2016} showed that treating first-call capture
...
\sct{Stevenson2016} proposed a new method for first-call capture
...
Fitting a model using this methodology is possible by specifying the
component \texttt{lower.cutoff} of \texttt{ss.opts} to be $c_l$. The
upper threshold, $c_u$, is still set using the \texttt{cutoff}
component. At present, these first-call models can only be fitted
assuming a linear relationship between expected received signal
strength and distance (Equation \eqref{eq:ss-link-identity}).
...
A description of call strength heterogeneity and directional calling
models can be found in \sct{Stevenson2016}.
