...
\texttt{admbsecr} package. This document also covers the fitting of
acoustic SECR models where detection not only depends on the distance
...
gibbons.fit <- admbsecr(capt = gibbons.capt, traps = gibbons.capt, 
                        mask = gibbons.mask)
@
...
lightfooti.fit <- admbsecr(capt = lightfooti$capt, traps = lightfooti$traps,
...
A simulation approach is currently the only appropriate way of
obtaining variance estimates for these data \scp{Stevenson2015}, and
this can be carried out using the \texttt{boot.admbsecr()}
function. For example,
<<lightfooti-code-2, eval = FALSE, tidy = FALSE>>=
lightfooti.boot <- boot.admbsecr(lightfooti.fit, N = 10, n.cores = 4)
@ 
...
may be implemented in a future version of \texttt{admbsecr}.

\section{First calls as detection units}

In the absence of existing methodology capable of estimating animal
density for call-based capture histories, one may instead elect to
define first calls as detected units. In their analysis of the
ovenbird \emph{Seiurus aurocapilla}, \sct{Dawson2009} used the
following protocol to decide which calls to include in analysis:
\begin{enumerate}
...
\texttt{secr} package. See the \emph{Data structure for
  \texttt{admbsecr}} vignette (run \texttt{vignette("data-structure")}
for information on how to convert the object from \texttt{secr} into
an appropriate object for analysis with \texttt{admbsecr}. The models
used by \sct{Dawson2009} can then be fitted with \texttt{admbsecr},
for example,
<<ovenbird-code, eval = FALSE, tidy = FALSE>>=
ovenbird.fit <- admbsecr(capt = ovenbird.capt, traps = ovenbird.traps, 
...
(i.e., if $B = \pi$). The \texttt{admbsecr} package supports the
...
Note that the model does not require knowledge of the directions calls
were emitted in. These directions can be estimated from the data,
though this performs best when either:
\begin{itemize}
...
\texttt{admbsecr()} function for further details. 
...
So far there has been no development of acoustic SECR methodology that
