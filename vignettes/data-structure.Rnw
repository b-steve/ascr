...
ovenbird.mask <- create.mask(traps = ovenbird.traps, buffer = 200)
@ 
...
\begin{itemize}
\item \texttt{Session}: Identifies the session (see Section
  \ref{sec:multi-session} below).
...
The third column of this data frame is ignored because \texttt{ascr}
does not support multi-occasion data. If additional information has
been collected by the detectors, then this information should appear
in subsequent columns with the appropriate names (\texttt{bearing},
\texttt{dist}, \texttt{ss}, or \texttt{toa}).
...
also be compatible with \texttt{ascr}. The functions
\texttt{create.mask()} (in \texttt{ascr}) and \texttt{make.mask()} (in
\texttt{secr}) are both capable of creating suitable mask objects.

\section{Multi-session data}
\label{sec:multi-session}

Some surveys collect data over multiple `sessions', which are assumed
to have been carried out independently of one another. For example,
multiple arrays with different numbers of detectors in different
spatial configurations may have been deployed at different locations
in space in such a way that the same individual will not be detected
by detectors in different arrays.

For surveys with more than one session, the \texttt{capt},
\texttt{traps}, and \texttt{mask} objects should be lists of the
matrices described above, one for each session. For example, if
\texttt{capt1} and \texttt{capt2} contain capture histories in the
format described above in Section \ref{sec:capt} for two different
sessions, then an appropriate multi-session object can be created as
follows:
<<multi-capt, eval = FALSE, tidy = FALSE>>=
multi.capt <- list(capt1, capt2)
@ 

The same approach can be used to construct multi-session
\texttt{traps} and \texttt{mask} objects, or alternatively the
\texttt{create.mask()} function can create a multi-session
\texttt{mask} object from a multi-session \texttt{capt} object.
