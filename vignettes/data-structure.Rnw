...
functions \texttt{convert.capt.admbsecr()} and
\texttt{convert.capt.secr()} convert capture history objects used by
\texttt{secr} to objects suitable for analysis with \texttt{admbsecr},
...
capt <- convert.capt.admbsecr(signalCH)
...
mask <- create.mask(traps = traps, buffer = 200)
@ 
