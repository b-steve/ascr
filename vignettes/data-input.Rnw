...
\title{Data input for \texttt{admbsecr}}
\date{}
...
In order to carry out analysis with \texttt{admbsecr}, three objects are required:
\begin{itemize}
...
Data input for the \texttt{admbsecr} package is designed to work
similarly to the procedures used by the \texttt{secr}
package. Currently (as of Version 1.1.0) functions exist to convert
data objects used by \texttt{admbsecr} to objects suitable for
analysis with \texttt{secr} (e.g., \texttt{convert.capt()},
\texttt{convert.mask()}, and \texttt{convert.traps()}), and
implementation of functions to perform the conversion in the other
...
As \texttt{admbsecr} does not (yet) support multi-session or
...
also be compatible with \texttt{admbsecr}.

The functions \texttt{create.mask()} (in \texttt{admbsecr}) and
