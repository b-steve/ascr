// separable function for a(theta) using 1 - pm trick.
DATA_SECTION
...
PARAMETER_SECTION
  ...
  init_bounded_number D(0,1e+07,-1)
  init_bounded_number ssb0(0,600)
  init_bounded_number ssb1(-15,0)
...
SEPARABLE_FUNCTION void ll_exp(const dvariable& D, const dvariable& ssb0, const dvariable& ssb1, const dvariable& sigmass, const dvariable& m1, const dvariable& m2)
  ...
  for (int j = 1; j <= ntraps; j++){
    ...
    log_p += log(cumd_norm((c - muss)/sigmass) + DBL_MIN);
  }
  // Probability of capture.

  // Contribution to the likelihood is: exp(-1 * \int D * Area * Pr(capture) * f(X) dX)
  // Here we need to give: log(D * Area * Pr(capture)) (divide by 10000 to convert to hectares).
  f -= log(D/10000);

  // Area of support of X = 80^2/10000 
  f -= log(square(80.0));
