  ...
  do_separable_stuff_laplace_approximation_block_diagonal(df1b2variable& ff)
{
  ...
  if (us>0)
  {
    ...
    if (separable_calls_counter==2)
    {
     ...
     //cout << "----------------- logdetH= " << f << endl;
      ...
      initial_df1b2params::cobjfun -= exp_laplace;	// -= is because ADMB does minimization,  exp(D*prod(cumnorm))
    }
    else
  ...
  for (i=1;i<=xs;i++)
  {
    int ii=lfe_index(i);
    if (separable_calls_counter==2)
      xadjoint(list(ii,1)) -= exp_laplace*(-local_xadjoint(i));    // -= is because ADMB does minimization, (-local_xadjoint) is differential in optimizaiton, does  exp(D*a(theta))   
    else
