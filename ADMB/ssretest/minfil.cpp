  ...
  do_separable_stuff_laplace_approximation_block_diagonal(df1b2variable& ff)
{
  ...
  if (us>0)
  {
    ...
    if (separable_calls_counter==2)
    {
      ...
      //exp_laplace = exp( - (value_f_hans+f));	// Exponential term
								// Has added us*0.9189385 which is 0.5*log(2*pi)*us
     //cout << "----------------- exp_laplace = " << exp_laplace << endl;
     //cout << "----------------- logdetH= " << f << endl;

      initial_df1b2params::cobjfun -= -exp_laplace;	// ADMB collects -ll
    }
    else
  ...
  for (i=1;i<=xs;i++)
  {
    int ii=lfe_index(i);
    if (separable_calls_counter==2)
      xadjoint(list(ii,1)) -= -exp_laplace*(-local_xadjoint(i));
    else
