/*
 * $Id: df1b2ghmult.cpp,v 1.1 2013/03/13 19:34:50 hskaug Exp $
 *
  ...
  function_minimizer * pmin)
{
 ...
 // Here need hooks for sparse matrix structures
  ...
  else
  {
   ...
   //cout << "multi=" << pmin->lapprox->multi_random_effects << endl;
   //cout << "ad_comm::no_ln_det_choleski_flag=" << ad_comm::no_ln_det_choleski_flag << endl;

   nsc=pmin->lapprox->num_separable_calls;

   //cout << "lapprox->gh->mi = "<< pmin->lapprox->gh->mi<< endl;
   dvariable vf=0.0;

   // begin do_gauss_hermite_integration_multi -----------------------------------------

   dvariable det1;
   if (allocated(block_diagonal_vhessian(1)))
   {
     det1=exp(0.5*ln_det_choleski(block_diagonal_vhessian(1)));
   }
   else 
    cout << "block_diagonal_vhessian(1) not allocated" << endl;
       

  multi_index & mi2=mi;
  dvector& w=pmin->lapprox->gh->w;
  ivector& lrea2 = *(pmin->lapprox->num_local_re_array);
  for (int isc=1;isc<=nsc;isc++)
  {
    ...
    do
    {
      ...
      int is=mi2.get_offset()+1;
      ftmp+=exp(-pmin->lapprox->gh->gauss_hermite_values(isc,is))*ww;
      ++mi2;
    }
    while(mi2.get_depth()<=lus);
    //cout << "isc=" << isc << " det1=" << det1 << " ftmp=" << ftmp << endl;

    if(isc==1)
      vf-= det1*ftmp;			// Dette er exp() leddet.		
    else
      vf-=log(ftmp);
  }

  // end do_gauss_hermite_integration_multi -----------------------------------------

   int sgn=0;
   dvariable ld=0.0;
   if (ad_comm::no_ln_det_choleski_flag)
   {
     for (int ic=2;ic<=nsc;ic++)
     {
