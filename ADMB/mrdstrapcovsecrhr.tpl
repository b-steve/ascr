...
PROCEDURE_SECTION
  ...
  for (i=1; i<=nmask; i++){
    ...
    maskp21(i)=g02*(1 - mfexp(-pow(dist(2,i)/sigma2,-z2)))+DBL_MIN;
  }
  maskp12=1-maskp11;
  maskp22=1-maskp21;
  ...
  for (i=1; i<=n; i++){
    indivprobs(i)=g01*(1 - mfexp(-pow(indivdist(i,1)/sigma1,-z1)))+DBL_MIN;
  }
  logprobs=elem_prod(log(indivprobs),column(capt,1))+elem_prod(log(1-indivprobs),column(1-capt,1));
  for (i=1; i<=n; i++){
    indivprobs(i)=g02*(1 - mfexp(-pow(indivdist(i,1)/sigma2,-z2)))+DBL_MIN;
  }
