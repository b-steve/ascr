sim_study = function(sim_name, n.rand = 1, fit = FALSE, seed = 810){
  ...
  output = list()
  output$sim_capt = simulated_capt
  ...
  if(fit){
    ...
    fit_args = fit_args_generator(sim_name, sim_args)
    ...
    if(n.rand == 1){
      fit_args$captures = simulated_capt
      sim_fit = do.call('fit.ascr', fit_args)
      ...
      for(i in 1:n.rand){
        fit_args$captures = simulated_capt[[i]]
        sim_fit = do.call('fit.ascr', fit_args)
