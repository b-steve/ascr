...
make.all.tpl.easy <- function(memory, method, detfn, parnames, scalefactors, cpi){
  ...
  ## Inserting code for Da estimation.
  if (!is.null(cpi)){
    codesplit <- strsplit(codestring, "//@SDREPD")[[1]]
    sdrepstr <- "  sdreport_number D"
    codestring <- splitinsert(codesplit, sdrepstr)
    codesplit <- strsplit(codestring, "//@SPECD")[[1]]
    specdstr <- "D = Da*muC;\n  for (i = 1; i <= nr; i++){\n    f -= -log(sigmaC) - log(sqrt(2*pi)) - square(cpi(i) - muC)/(2*square(sigmaC));\n  }"
    codestring <- splitinsert(codesplit, specdstr)
  }
...
get.tpl.text <- function(methods = c("simple", "toa", "ang", "ss", "sstoa", "dist", "angdist", "mrds", "ssmrds"),
