...
test_that("simple model bootstrapping", {
    ...
    boot.fit <- boot.ascr(example.data$fits$simple.hn, N = 5, prog = FALSE)
    ...
    ## Coefficients.
    expect_that(coef(boot.fit), is_identical_to(coef(example.data$fits$simple.hn)))
    ## Standard errors.
    ...
    library(parallel)
    set.seed(8871)
    boot.fit.par <- boot.ascr(example.data$fits$simple.hn, N = 5, prog = FALSE,
                                  n.cores = detectCores())
...
test_that("cue-rate model bootstrapping", {
    ...
    ses.test <- c(305.466627386732, 5.03347019909088,
                  ...
                  0.399775773640889, 0.0291562697985857,
                  0.223637444167788, 0.108387339783415,
                  0.350108662810472)
...
test_that("bootstrapping helpers", {
    ## MCE extraction.
    expect_that(ascr:::get.mce(example.data$fits$boot.simple.hn, "se"),
                equals(example.data$fits$boot.simple.hn$boot$se.mce))
    ## Variance-covariance matrix extraction.
    expect_that(sort(vcov(example.data$fits$boot.simple.hn, "all")[c(4, 5, 4, 2, 3, 1), c(4, 5, 4, 2, 3, 1)]),
                equals(sort(example.data$fits$boot.simple.hn$boot$vcov)))
    ## Extraction of bias.
    expect_that(get.bias(example.data$fits$boot.simple.hn, c("D", "esa")),
                equals(example.data$fits$boot.simple.hn$boot$bias[c("D", "esa.1")]))
    ## Coefficient extraction with bias correction.
    expect_that(coef(example.data$fits$boot.simple.hn, correct.bias = TRUE),
                equals(coef(example.data$fits$boot.simple.hn) - get.bias(example.data$fits$boot.simple.hn)))
    ## Confidence interval methods.
    ## Default.
    cis.test <- matrix(c(1420.53146733787, 4.3550748265765, 3114.94748365933, 6.42514893964572),
                       nrow = 2)
    cis <- confint(example.data$fits$boot.simple.hn)
